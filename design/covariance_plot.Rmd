...
h_vectorization <- function(vcov_matrix, string = NULL) {
  ...
  vcov_matrix_row_label <- unlist(strsplit(names(vect), ":"))[seq(1, length(vect) * 2, by = 2)] ## hyperfuntion #lab.t1
  vcov_matrix_col_label <- unlist(strsplit(names(vect), ":"))[seq(2, length(vect) * 2, by = 2)] #lab.t2
  if (is.null(string)) {
...
#' @examples
#' g_covariance(vcov_dataframe = h_vectorization(vcov_matrix=cholest.cov), x.var="tp.dist")
g_covariance <- function(vcov_dataframe, x.var = c("lag", "tp.dist"), ylab = "",
  ...
  assert_true(x.var == "lag" & is.null(xlab)) {
    xlab <- "Lag"
  }
  assert_true(x.var == "tp.dist" & is.null(xlab)) {
    xlab <- "Distance (time units) btw measurements"
  }
  n.col <- length(unique(vcov_dataframe$lag))
  ntp <- 1:max(vcov_dataframe$rnk.t1)
  if (length(col) < n.col) col <- rep(col, n.col)
  if (is.null(pch)) pch <- ntp
  if (is.null(lty)) lty <- ntp
  legend.pos <- match.arg(legend.pos)
  
  ggplot(vcov_dataframe, aes(vcov_dataframe[, x.var], vcov_dataframe[, "Vect"], colour = as.factor(vcov_dataframe[, "rnk.t1"]), group = as.factor(vcov_dataframe[, "rnk.t1"]))) +
    geom_point() +
    geom_line() +
