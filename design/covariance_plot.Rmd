...
h_vectorization  <- function(d.matrix,string=NULL){
  if(!is.matrix(d.matrix)){ stop("d.matrix is not a matrix")
  } else {
    if(!isSymmetric(d.matrix)) stop("d.matrix should be a symetric matrix")  
  }
  ...
  if(is.null(string)) {
    ...
    if(!any(grepl(string,cnames))) { stop("The string you entered is not part of the column/row names of d.matrix")
    } else{
...
{
  tre.col <- c("#0080ff","#ff00ff","darkgreen","#ff0000","orange",
               "#00ff00","brown")
  ...
  ggplot2::ggplot(d.cov, aes(d.cov[,x.var], d.cov[,"Vect"], colour = as.factor(d.cov[,"rnk.t1"]), group = as.factor(d.cov[,"rnk.t1"]))) + geom_point() + geom_line() + labs(colour = "From time:", x = xlab, y = ylab)
}
```
